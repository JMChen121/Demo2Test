"""Uses GAIL to training an attack policy against a fixed, embedded policy."""
...
from aprl.policies.loader import load_policy, mpi_unavailable_error
from aprl.policies.wrappers import MultiPolicyWrapper
from aprl.pytorch_rl import wrappers_gairl
from aprl.pytorch_rl.gairl import GAIRL
...
def _save(model, root_dir: str, save_callbacks: Iterable[SaveCallback]) -> None:
    ...
    model_path = osp.join(root_dir, "final_model.pkl")
...
def _stable(
    ...
    if load_policy["path"] is not None:
        # TODO: Resume from saved model
        pass
    else:
        # model = cls(policy=policy, seed=_seed, **kwargs)
        # get model from pytorch-rl
        # seed: 3407, 114514
        model = cls(env=env, expert_dataset=kwargs["expert_dataset"], out_dir=out_dir, seed=3407, agent_id=1-embed_index)
    # TODO: train model and save model by pytorch-rl
    model.learn()
...
@train_ex.capture
def gairl(batch_size, learning_rate, expert_dataset_path, **kwargs):
    ...
    expert_dataset = ExpertDatasetFromOurFormat(expert_dataset_path, batch_size=2048)
...
def train_config():
    # Environment
    game_name = "SumoAnts-v0"
    # Embedded Agent Config
    # Typically this is the victim, but for victim hardening this could be the adversary
    embed_index = 1  # index embedded agent plays as
    embed_type = "zoo"  # any type supported by aprl.policies.loader
    embed_path = "1"  # path or other unique identifier
    embed_types = None  # list of types for embedded agents
    embed_paths = None  # list of paths for embedded agents

    expert_dataset_path = 'data/trajectories/RunToGoalAnts_source_traj.npz'  # path of source trajectory data to train demo2test

    # expert_dataset_path = 'data/trajectories/2023-09-09-17-32-57_YouShallNotPassHumans/agent_0_win.npz'
    # embed_index = 0  # index embedded agent plays as
    # embed_path = "3"
    # game_name = "KickAndDefend-v0"

    env_name = f"multicomp/{game_name}"  # Gym environment ID
    ...
    # rl_algo = "ppo"  # RL algorithm to use
    rl_algo = "gairl"  # RL algorithm to use
    ...
    rl_args = dict() if rl_algo == "gail" else dict(ent_coef=0.0, nminibatches=4,
                                                    noptepochs=4)  # algorithm-specific arguments for ppo2

    # Logging
    root_dir = "data/baselines"  # root of directory to store baselines log
    exp_name = f"{rl_algo}_{game_name}"  # name of experiment
...
def wrappers_config(env_name):
    ...
    rew_shape_params.update(dict(anneal_frac=0.0))
...
def build_env(
    ...
    def env_fn(i):
        return wrappers_gairl.make_env(
            env_name,
            _seed,
            i,
            out_dir,
