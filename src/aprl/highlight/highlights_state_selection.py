...
def highlights(state_importance_df, budget, context_length, minimum_gap, importance_title='importance'):
    ...
    we chose state 200, and the context length is 10, we will show states 189-211. If minimum_gap=10, we will not
    consider states 212-222 and states 178-198 because they are too close
    :param importance_title: title of importance column
    :return: a list with the indices of the important states, and a list with all summary states (includes the context)
    '''
    sorted_df = state_importance_df.sort_values([importance_title], ascending=False)
    summary_states = []
...
def highlights_div(state_importance_df, budget, context_length, minimum_gap, distance_metric=distance.euclidean,
                   percentile_threshold=3, subset_threshold=1000, importance_title='importance'):
...
def read_input_files(path):
    ...
    state_input_df = pd.DataFrame(
        {'state': states, 'features': input_list})  # we use features as name to make the div-code less bloated
...
if __name__ == '__main__':
    ...
    # image_utils.generate_video('stream/argmax_smooth/','stream/','highlights_div_15_10_10.mp4', image_indices=summary_states_with_context)
    summary_states, summary_states_with_context = highlights(state_features_importance_df, 15, 10, 10)
    print('reg', summary_states)
    # image_utils.generate_video('stream/argmax_smooth/','stream/','highlights_reg_15_10_10.mp4', image_indices=summary_states_with_context)
    # exit()
